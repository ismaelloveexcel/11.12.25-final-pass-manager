...
This project is a single static `index.html`, so the simplest and most reliable portal to deploy is **GitHub Pages**:
- In the repository settings, enable GitHub Pages with the branch set to `main` (or your default branch) and root set to `/` so `index.html` is served.
